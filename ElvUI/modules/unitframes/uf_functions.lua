...
E.LoadUFFunctions = function(layout)
	...
	function E.ConstructCastBar(self, width, height, direction)
		...
		if C["unitframes"].cbicons == true then
			...
			castbar.Icon:Width(height)
			castbar.Icon:Height(height)
			...
			if direction == "LEFT" then
				...
				castbar.Icon:Point("RIGHT", castbar, "LEFT", -8*E.ResScale, 0)
			else
				--button:Point("LEFT", castbar, "RIGHT", 6*E.ResScale, 0)
				castbar.Icon:Point("LEFT", castbar, "RIGHT", 8*E.ResScale, 0)
			end
			button:Point("TOPRIGHT", castbar.Icon, "TOPRIGHT", 2*E.ResScale, 2*E.ResScale)
			button:Point("BOTTOMLEFT", castbar.Icon, "BOTTOMLEFT", -2*E.ResScale, -2*E.ResScale)
			...
			castbar:Width(width - castbar.Icon:GetWidth() - 6)
		end
				...
				if min ~= max then
					...
					if unit == "player" and health:GetAttribute("normalUnit") ~= "pet" then
						if C["unitframes"].showtotalhpmp == true then
							health.value:SetFormattedText("|cff559655%s|r |cffD7BEA5|||r |cff559655%s|r", min, max)
						else
							health.value:SetFormattedText("|cffAF5050%s|r\n|cff%02x%02x%02x%.1f%%|r", min, r * 255, g * 255, b * 255, (min / max * 100))
						end
					...
					elseif (unit and unit:find("boss%d")) then
						if C["unitframes"].showtotalhpmp == true then
							...
							health.value:SetFormattedText("|cffAF5050%s|r |cffD7BEA5-|r |cff%02x%02x%02x%.1f%%|r", min, r * 255, g * 255, b * 255, (min / max * 100))
						end
					elseif (unit and unit:find("arena%d")) then
						health.value:SetText("|cff559655"..E.ShortValue(min).."|r")
					else
						health.value:SetFormattedText("|cff%02x%02x%02x%.1f%%|r", r * 255, g * 255, b * 255, (min / max * 100))
					end
				else
					if unit == "player" and health:GetAttribute("normalUnit") ~= "pet" then
					...
					elseif unit == "target" or (unit and unit:find("arena%d")) then
						health.value:SetText("|cff559655"..max.."|r")
					elseif unit == "targettarget" or unit == "pet" or unit == "pettarget" or unit == "focustarget" or unit == "focus" then
						health.value:SetText("")
					else
						health.value:SetText("|cff559655"..max.."|r")
					end
					...
					if pType == 0 then
						if unit == "target" then
							if C["unitframes"].showtotalhpmp == true then
								...
								power.value:SetFormattedText("%s\n%d%%", max - (max - min), floor(min / max * 100))
							end
