...
local function CheckFade(self, elapsed)
	...
	if E.db["meat"].microbar.mouseover ~= true then return end
...
function AB:UpdateMicroBar()
	...
	if E.db["meat"]["microbar"].enable ~= true then
		f:SetAlpha(0);
		f:SetScale(0.000001);
	else
		f:SetAlpha(1);
	...
	if E.db['meat']['microbar'].mouseover == true or E.db["meat"]["microbar"].enable == false then
		f:SetAlpha(0)
	else
		f:SetAlpha(1)
	end
